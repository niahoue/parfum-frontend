// src/pages/cosmetiques/CosmeticsPage.jsx
...
const CosmeticsPage = () => {
  ...
  useEffect(() => {
    const fetchCosmetics = async () => {
      ...
      try {
        ...
        // Récupérer tous les produits de la catégorie "Cosmétique"
        ...
        console.error('Erreur lors du chargement des produits cosmétiques:', err);
        setError(err.response?.data?.message || 'Échec du chargement des produits cosmétiques.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center">Cosmétiques</h1>
      <p className="text-xl text-gray-600 mb-8 text-center">
        Découvrez notre gamme complète de produits cosmétiques pour sublimer votre beauté.
      </p>

      {loading ? (
        <div className="text-center py-10">
          <Loader2 className="animate-spin inline-block mr-2" />Chargement des produits cosmétiques...
        </div>
      ) : error ? (
        <div className="text-center py-10 text-red-600">
          <p className="text-lg">{error}</p>
          {error.includes("Catégorie") && (
            <p className="mt-4 text-gray-700">Veuillez vous assurer qu'une catégorie nommée "Cosmétique" existe et contient des produits.</p>
          )}
        </div>
      ) : products.length === 0 ? (
        <div className="text-center text-gray-600 py-10">
          Aucun produit cosmétique trouvé pour le moment.
        </div>
      ) : (
