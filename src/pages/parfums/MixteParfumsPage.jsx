// src/pages/parfums/MixteParfumsPage.jsx
...
const MixteParfumsPage = () => {
  ...
  useEffect(() => {
    const fetchMixtePerfumes = async () => {
      setLoading(true);
      try {
        ...
        // Récupérer tous les produits de la catégorie "Mixte"
        const { data } = await axiosClient.get('/products', {
          params: {
            category: mixteCategory._id // Filtrer par l'ID de la catégorie "Mixte"
          }
        });
        setProducts(data.products || []);
        setError(null);
      } catch (err) {
        console.error('Erreur lors du chargement des parfums mixtes:', err);
        setError(err.response?.data?.message || 'Échec du chargement des parfums mixtes.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center">Parfums Mixte</h1>
      <p className="text-xl text-gray-600 mb-8 text-center">
        Découvrez notre sélection complète de parfums unisexes et mixtes.
      </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-red-600">
          ...
          {error.includes("Catégorie") && (
            <p className="mt-4 text-gray-700">Veuillez vous assurer qu'une catégorie nommée "Mixte" existe et contient des produits.</p>
          )}
        </div>
      ) : products.length === 0 ? (
        <div className="text-center text-gray-600 py-10">
          Aucun parfum mixte trouvé pour le moment.
        </div>
      ) : (
...
export default MixteParfumsPage;
