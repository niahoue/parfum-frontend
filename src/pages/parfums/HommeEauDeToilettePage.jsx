// src/pages/parfums/HommeEauDeToilettePage.jsx
...
const HommeEauDeToilettePage = () => {
  ...
  useEffect(() => {
    const fetchEauDeToilette = async () => {
      ...
      try {
        ...
        // Récupérer les produits qui sont de type "Eau de Toilette" et de la catégorie "Homme"
        ...
        console.error('Erreur lors du chargement des eaux de toilette homme:', err);
        setError(err.response?.data?.message || 'Échec du chargement des eaux de toilette homme.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center">Eau de Toilette Homme</h1>
      <p className="text-xl text-gray-600 mb-8 text-center">
        Découvrez notre sélection d'eaux de toilette fraîches pour homme.
      </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-red-600">
          ...
          {error.includes("Catégorie") && (
            <p className="mt-4 text-gray-700">Veuillez vous assurer qu'une catégorie nommée "Homme" existe et que des produits de type "Eau de Toilette" lui sont associés.</p>
          )}
        </div>
      ) : products.length === 0 ? (
        <div className="text-center text-gray-600 py-10">
          Aucune eau de toilette homme trouvée pour le moment.
        </div>
      ) : (
...
export default HommeEauDeToilettePage;
