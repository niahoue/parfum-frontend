// src/pages/parfums/HommeNouveautesPage.jsx
...
const HommeNouveautesPage = () => {
  ...
  useEffect(() => {
    const fetchNewArrivals = async () => {
      ...
      try {
        ...
        // Récupérer les produits qui sont des "nouveautés" et de la catégorie "Homme"
        ...
        console.error('Erreur lors du chargement des nouveautés homme:', err);
        setError(err.response?.data?.message || 'Échec du chargement des nouveautés parfums homme.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center">Nouveautés Parfums Homme</h1>
      <p className="text-xl text-gray-600 mb-8 text-center">
        Découvrez les dernières fragrances masculines ajoutées à notre collection.
      </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-red-600">
          ...
          {error.includes("Catégorie") && (
            <p className="mt-4 text-gray-700">Veuillez vous assurer qu'une catégorie nommée "Homme" existe et que des produits marqués comme "nouveaux" lui sont associés.</p>
          )}
        </div>
      ) : products.length === 0 ? (
        <div className="text-center text-gray-600 py-10">
          Aucune nouveauté parfum homme trouvée pour le moment.
        </div>
      ) : (
...
export default HommeNouveautesPage;
